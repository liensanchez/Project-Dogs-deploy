...
const allDogsInfo = async () => {
    ...
    const completeDog = dogApi.map((dog) => {
        if (dog.name == "smooth fox terrier")
            (dog.weight = ["6", "8"]), (dog.weightMin = 6);
        if (dog.name == "pekingese")
            (dog.weight = ["3", "6"]), (dog.weightMin = 3);
        if (dog.name == "french bulldog")
            (dog.weight = ["9", "13"]), (dog.weightMin = 9);
        if (dog.name == "olde english bulldogge")
            (dog.weight = ["22", "30"]), (dog.weightMin = 22);
